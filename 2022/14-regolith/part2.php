...
for($count=0; $count < 100_000; $count++) {
	...
	// Is the spawner blocked?
	$is_spawner_blocked = ($sand[0] == 500 && $sand[1] == 0);
	
	if($is_spawner_blocked)
		$count++;
	
	if(GENERATE_ANIMATION) {
		$frame_rate = match(true) {
			$count > 20_000 => 200,
			$count > 10_000 => 150,
			$count > 1_000 => 100,
			$count > 100 => 10,
		...
		if ((0 == $count % $frame_rate) || $is_spawner_blocked) {
			$frame = imagescale($im, ($width - $extents[0]) * 2, $height * 2, IMG_NEAREST_NEIGHBOUR);
			$color_output = imagecolorallocate($frame, 255, 255, 255);
			imagettftext($frame, 15, 0, 600, 35, $color_output, 'font/Roboto-Medium.ttf', 'Part 2: ' . $count);
			imagettftext($frame, 8, 0, 600, 50, $color_output, 'font/Roboto-Medium.ttf', '@jeff@phpc.social');
			imagepng($frame, sprintf('animation/cave_%05d.png', $count));
			imagedestroy($frame);
		}
	}
	
	// If we blocked the sand spawner, stop making sand
	if($is_spawner_blocked)
		break;
}

// Visualize the rock barriers
for($y=0;$y<=$height;$y++) {
...
if(GENERATE_IMAGE) {
	$final = imagescale($im, ($width - $extents[0]) * 2, $height * 2, IMG_NEAREST_NEIGHBOUR);
	$color_output = imagecolorallocate($final, 255, 255, 255);
	imagettftext($final, 15, 0, 600, 35, $color_output, 'font/Roboto-Medium.ttf', 'Part 2: ' . $count);
	imagettftext($final, 8, 0, 600, 50, $color_output, 'font/Roboto-Medium.ttf', '@jeff@phpc.social');
	imagepng($final, 'part2.png');
	imagedestroy($im);
	imagedestroy($final);
}